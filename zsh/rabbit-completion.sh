...
_rabbitmqadmin()
{
    ...
    case "${prev}" in
        list)
            COMPREPLY=( $(compgen -W 'connections channels consumers exchanges queues bindings users vhosts permissions nodes parameters policies operator_policies vhost_limits' -- ${cur}) )
            return 0
            ;;
        show)
            ...
            ;;
        declare)
            COMPREPLY=( $(compgen -W 'exchange queue binding vhost user permission parameter policy operator_policy vhost_limit' -- ${cur}) )
            return 0
            ;;
        delete)
            COMPREPLY=( $(compgen -W 'exchange queue binding vhost user permission parameter policy operator_policy vhost_limit' -- ${cur}) )
        ...
        --username)
            ...
            ;;
        -f)
            COMPREPLY=( $(compgen -W "raw_json pretty_json tsv long table kvp bash"  -- ${cur}) )
            return 0
            ;;
        --format)
            COMPREPLY=( $(compgen -W "raw_json pretty_json tsv long table kvp bash"  -- ${cur}) )
        ...
        policie)
            ...
            ;;
        operator_policie)
            opts="$(rabbitmqadmin -q -f bash list operator_policies)"
            COMPREPLY=( $(compgen -W "${opts}"  -- ${cur}) )
            return 0
            ;;
        vhost_limit)
            opts="$(rabbitmqadmin -q -f bash list vhost_limits)"
            COMPREPLY=( $(compgen -W "${opts}"  -- ${cur}) )
            return 0
            ;;
        *)
